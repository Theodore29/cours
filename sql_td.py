...
conn=sqlite3.connect('magasin21.db')
cur=conn.cursor()
...
ex_3_produit5 = "INSERT INTO produit VALUES (39,  'Bsc Ptt Lycéen', 2.45)"
...
ex_3_produit14 = "INSERT INTO produit VALUES (70, 'Pain épice miel', 6.16)"
...
##Question 4:
ex4_q1="SELECT nom,prenom FROM client WHERE cp = 25000" # Les personnes qui habitent à Besancon sont Bertrand Patrice et Marcheur Luc
cur.execute(ex4_q1)

ex4_q2="SELECT prix FROM produit WHERE nom_prod ='Baguette Rust'" # Une baguette Rust coute 0,9€
cur.execute(ex4_q2)

ex4_q3="UPDATE produit SET prix=0.95 WHERE code_prod=37"
cur.execute(ex4_q3)

ex4_q4="SELECT num_carte FROM client WHERE nom='Ciel' AND prenom='Leila' " # le numéro de client de Leila Ciel est 900000001
cur.execute(ex4_q4)
...
ex4_q6="SELECT * FROM client ORDER BY nom " #La table client est désormais triée par nom
cur.execute(ex4_q6)

ex4_q7="SELECT COUNT(*) mode_reg FROM ticket WHERE mode_reg='CB' " #Il y a 3 clients qui ont réglés en CB
cur.execute(ex4_q7)

ex4_q8="SELECT COUNT(*) mode_reg FROM ticket WHERE mode_reg <> 'CHEQUE'" #Il y a 4 client qui n'ont pas payé en chèque
cur.execute(ex4_q8)
...
cur.execute(ex4_q9) #Madame Dupont à fait ses courses à 17h54
...
cur.execute(ex4_q10) #Les clients qui ont achétés uen Baguette Rust sont Hamelin Jules, Marcheur Luc et Bertrand Patrice.


ex4_q11="SELECT produit.prix FROM produit JOIN possede ON client.identifiant = possede.identifiant JOIN ticket ON possede.num_ticket = ticket.num_ticket JOIN achat ON ticket.num_ticket = achat.num_ticket JOIN produit ON achat.code_produit = produit.code_prod WHERE client.nom ='Marcheur' AND client.prenom = 'Luc' "
cur.execute(ex4_q11)
